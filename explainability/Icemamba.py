...
class SelectiveScan(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1):
        ...
        if SSMODE == "mamba_ssm":
            ...
            # delta_softplus = delta_softplus.to(device)
            # nrows = nrows.to(device)
...
class VSSM(nn.Module):
    def __init__(
        ...
        self.output = nn.Conv2d(int(self.decoder_VSSD[-1]), self.num_forcast, kernel_size=1, bias=False)
...
if __name__ == "__main__":
    model = VSSM(patchembed_version='v1', downsample_version='v1')
    summary(model, (41, 448, 304))
